...
exports.addDataCallback = function(func) {
	if (dataListeners.indexOf(func)) {
		dataListeners.push(func);
	}
}
...
var attemptConnection = function(port)
{
	...
	arduino = new serialport.SerialPort(port, { baudrate: 57600, parser: serialport.parsers.readline("\n") });
	arduino.on("open", function () {
		console.log('* connection to arduino successful ! *');
		arduino.on('data', function(data){
			for (var i = 0;i < dataListeners.length; i++) {
				var func = dataListeners[i];
				func(data);
			}
